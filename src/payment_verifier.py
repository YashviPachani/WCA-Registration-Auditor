...
def read_amount_from_screenshot(image_path, expected_amount=None):
    ...
    if amount is None:
        return {"amount": None, "confidence": "low", "notes": "Could not detect amount in screenshot"}
...
def verify_payment_screenshot(person_key, drive_links, expected_amount):
    ...
    for link in drive_links:
        ...
        ocr_result = read_amount_from_screenshot(local_path, expected_amount=expected_amount)
...
def verify_all_screenshots(matched_df, screenshot_links_column="screenshot_links", progress_callback=None):
    """
